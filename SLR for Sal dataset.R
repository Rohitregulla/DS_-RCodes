...
library(readr)
Sal_dat<- read_csv("/Users/rohitregulla/Downloads/Salary_Data.csv")
View(Sal_dat)
...
plot(Sal_dat$YearsExperience , Sal_dat$Salary)
plot(Sal_dat$YearsExperience , Sal_dat$Salary)
...
mean(reg$residuals)
sqrt(sum(reg$residuals^2)/nrow(Sal_dat))  #RMSE
...
ggplot(data = Sal_dat, aes(x = YearsExperience, y = Salary)) + 
  geom_point(color='blue') +
  geom_line(color='red',data = Sal_dat, aes(x=YearsExperience, y=pred))
